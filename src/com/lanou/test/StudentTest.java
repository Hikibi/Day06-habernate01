...
public class StudentTest {
    ...
    //    session 的工厂类对象, 是线程安全的, 一个数据库对应一个 session 工厂类对象
//    通常在项目启动时初始化该对象, 用 Configuration 对象创建
    private SessionFactory sessionFactory;

    //    真正执行 CRUD 操作的数据库连接对象, 代表了一次数据库连接, 是非线程安全的
    private Session session;

    //    事务对象, 与 jdbc 的事务类似, 只不过 hibernate 中的事务是不自动提交的, 而 jdbc 中的
//    事务是会自动提交, hibernate 中的事务需要手动提交
    private Transaction transaction;


    /**
     * 初始化操作
     * Before 注解 : 在 Test 注解之前执行的方法, 通常该方法中做一些初始化的操作
     */
    @Before
    public void init() {
...
//        通过配置对象创建一个 sessionFactory 对象 创建结束之后 configuration
//        就与 sessionFaction 失去关联了
...
//        开启事务
    ...
    /**
     * 销毁操作
     * After 注解 : 在 Test 注解之后执行的方法, 通常在该方法中做一些释放, 关闭操作
     */
    @After
    public void destroy() {
...
//        条件语句中的占位符参数对应
        query.setString(0, "张三");   // 指定第 1 个问号所对应的值
